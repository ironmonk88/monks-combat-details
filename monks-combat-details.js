...
export class MonksLittleDetails {
    ...
    static init() {
        ...
        if (!game.modules.get("monks-wall-enhancement")?.active) {
            ...
            //+++ double-click to split wall segment
            //WallsLayer.prototype._onClickLeft2 = function (event) {
            //
            //}

            //+++ Try to find matches for points that are really close.
        }
    }
    ...
    static ready() {
        ...
        if (!game.modules.get("monks-wall-enhancement")?.active) {
            if (setting('show-drag-points-together')) {
                ui.notifications.info('The option to drag wall points together is moving to a new module and will be disabled in Little Details shortly.  Please use Monks Wall Enhancement');
                console.warn('The option to drag wall points together is moving to a new module and will be disabled in Little Details shortly.  Please use Monks Wall Enhancement');
            }
        }
    }
